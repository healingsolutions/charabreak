...
const TEXT_LANDING_BACKSTEP = 6;
const TEXT_LANDING_FORWARD_PAD = 12;
const BLOOM_GLYPHS = ['\u273f', '\u2740', '\u2735', '\u2726'];
...
export class TextBreaker {
    ...
    bloomAtRect(rect, options = {}) {
        ...
        const bloomed = candidates.slice(0, limit).map((candidate) => this.bloomSpan(candidate.span, candidate.rect, options.source || 'player'));

        return {
            count: bloomed.length,
            chars: bloomed.map((item) => item.char),
            items: bloomed,
            critical: false,
            criticalWord: '',
            target: bloomed[0]?.target || null,
    ...
    findLandingY(runnerRect, previousBottom, nextBottom) {
        return this.findTextSurfaceY(
            runnerRect,
            (rect) => rect.top >= previousBottom - TEXT_LANDING_BACKSTEP && rect.top <= nextBottom + TEXT_LANDING_FORWARD_PAD
        );
    }
    ...
    wrapElementText(element) {
        ...
        for (const node of nodes) {
            if (this.maxChars > 0 && this.charSpans.length >= this.maxChars) {
                break;
            }
    ...
    wrapTextNode(node, ownerElement) {
        ...
        for (let index = 0; index < chars.length; index += 1) {
            ...
            if (this.maxChars > 0 && this.charSpans.length >= this.maxChars) {
                const rest = document.createTextNode(chars.slice(index).join(''));
                fragment.appendChild(rest);
                insertedNodes.push(rest);
                break;
            }
    ...
    bloomSpan(span, rect, source = 'player') {
        const char = span.dataset.gwChar || span.textContent || '';
        const color = bloomColorForChar(char);
        span.dataset.gwBroken = '1';
        span.dataset.gwBrokenBy = `${source}_bloom`;
        span.dataset.gwBloomed = '1';
        span.dataset.gwBloomedBy = source;
        span.style.width = `${Math.max(rect.width, 6)}px`;
        span.style.minWidth = `${Math.max(rect.width, 6)}px`;
        span.style.height = `${Math.max(rect.height, 8)}px`;
        ...
        const timer = window.setTimeout(() => {
            ...
            span.classList.add('gw-break-char--bloomed');
            this.breakTimers.delete(timer);
            const fadeTimer = window.setTimeout(() => {
                span.textContent = '';
                span.classList.remove('gw-break-char--bloomed');
                span.classList.add('gw-break-char--broken');
                span.setAttribute('aria-hidden', 'true');
                this.breakTimers.delete(fadeTimer);
            }, 420);
            this.breakTimers.add(fadeTimer);
        }, 140);
...
function bloomGlyphForChar(char = '') {
    ...
    return BLOOM_GLYPHS[Math.abs(code) % BLOOM_GLYPHS.length];
}
