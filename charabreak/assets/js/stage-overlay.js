...
const UI_TEXT_JA = {
    ...
    controlGuidePad: '\u30b2\u30fc\u30e0\u30d1\u30c3\u30c9\u30fb\u30b9\u30de\u30db\u64cd\u4f5c\u3082OK',
    gamepadConnected: '\u30b2\u30fc\u30e0\u30d1\u30c3\u30c9\u3092\u63a5\u7d9a\uff01',
    gamepadDisconnected: '\u30b2\u30fc\u30e0\u30d1\u30c3\u30c9\u304c\u5916\u308c\u305f',
    touchGuide: '\u30b9\u30de\u30db: \u30bf\u30c3\u30d7\u653b\u6483 / \u4e0a\u30b9\u30ef\u30a4\u30d7\u30b8\u30e3\u30f3\u30d7 / \u6a2a\u3086\u3063\u304f\u308a\u79fb\u52d5 / \u6a2a\u3059\u3070\u3084\u304f\u30d1\u30ea\u30a3 / \u4e0b\u30b9\u30ef\u30a4\u30d7\u96e2\u3057\u3067\u305f\u3081\u653b\u6483',
    touchParry: '\u30d1\u30ea\u30a3\uff01',
    introTitle: '\u30da\u30fc\u30b8\u3092\u8db3\u5834\u306b\u3057\u3066\u9032\u3082\u3046',
    introLead: '\u6587\u5b57\u306b\u7740\u5730\u3057\u3066\u3001\u753b\u50cf\u3084\u30dc\u30bf\u30f3\u3092\u6e21\u308a\u306a\u304c\u3089\u4e0b\u3078\u3002\u5b9d\u7bb1\u30923\u3064\u958b\u3051\u308b\u3068GOAL\u304c\u8fd1\u3065\u304d\u307e\u3059\u3002',
    introKeys: 'J: \u653b\u6483 / K\u30fbX: \u6587\u5b57\u3092\u6301\u3064\u30fb\u6295\u3052\u308b / L: \u76fe / Space: \u30b8\u30e3\u30f3\u30d7',
    introPad: '\u30b2\u30fc\u30e0\u30d1\u30c3\u30c9\u30fb\u30b9\u30de\u30db\u64cd\u4f5c\u3082\u4f7f\u3048\u307e\u3059',
    introBadge: '\u30df\u30c3\u30b7\u30e7\u30f3\u958b\u59cb',
    introStart: '\u958b\u59cb',
    bgmChoiceTitle: 'BGM\u3092\u9078\u3093\u3067\u30b9\u30bf\u30fc\u30c8',
    bgmChoiceLead: 'BGM\u306f\u3042\u306a\u305f\u304c\u9078\u3093\u3060\u6642\u3060\u3051\u9cf4\u308a\u307e\u3059\u3002\u3042\u3068\u304b\u3089\u753b\u9762\u4e0a\u90e8\u306eBGM\u30dc\u30bf\u30f3\u3067\u3082\u5207\u308a\u66ff\u3048\u3067\u304d\u307e\u3059\u3002',
    bgmChoiceNote: '\u52b9\u679c\u97f3\u306f\u64cd\u4f5c\u306e\u624b\u3054\u305f\u3048\u3068\u3057\u3066\u5c0f\u3055\u304f\u9cf4\u308a\u307e\u3059\u3002',
    bgmChoiceOff: 'BGM\u306a\u3057\u3067\u958b\u59cb',
    bgmChoiceOn: 'BGM\u3042\u308a\u3067\u958b\u59cb',
    introDrop: '\u4e0a\u304b\u3089\u3044\u304f\u3088\uff01',
...
const FLOWER_ARROW_SPEED = 1040;
const FLOWER_ARROW_LIFETIME = 1300;
...
const TOUCH_SINGLE_TAP_DELAY_MS = 170;
const TOUCH_DOUBLE_TAP_MS = 300;
const TOUCH_TAP_MAX_MOVE = 16;
const TOUCH_MOVE_HOLD_PX = 24;
const TOUCH_SWIPE_MIN_PX = 46;
const TOUCH_SWIPE_MAX_MS = 280;
const TOUCH_PARRY_MIN_PX = 74;
const TOUCH_PARRY_MAX_MS = 220;
...
const BOSS_MISSILE_MAX_ACTIVE = 2;
const BOSS_MISSILE_MAX_ACTIVE_MOBILE = 1;
...
export class StageOverlay {
    constructor(options = {}) {
        ...
        this.playStyle = normalizePlayStyle(options.playStyle || 'break');
        ...
        this.mobileGameScale = clampNumber(options.mobileGameScale || 1, 0.65, 1);
        this.reducedMotion = Boolean(options.reducedMotion);
        this.onExit = options.onExit || (() => {});
        this.onRetry = options.onRetry || (() => {});
        this.onInventoryOpen = options.onInventoryOpen || (() => {});
        this.onBgmChoice = options.onBgmChoice || (() => {});
        ...
        this.handleStagePointerDown = this.handleStagePointerDown.bind(this);
        this.handleStagePointerMove = this.handleStagePointerMove.bind(this);
        this.handleStagePointerUp = this.handleStagePointerUp.bind(this);
        this.handleStagePointerCancel = this.handleStagePointerCancel.bind(this);
        this.handleStageTouchStart = this.handleStageTouchStart.bind(this);
        this.handleStageTouchMove = this.handleStageTouchMove.bind(this);
        this.handleStageTouchEnd = this.handleStageTouchEnd.bind(this);
        this.handleStageTouchCancel = this.handleStageTouchCancel.bind(this);
    ...
    mount() {
        ...
        this.startGamepadProbe();
        window.CharaBreakGamepadDebug = () => this.describeGamepadState();
    ...
    destroy() {
        ...
        this.stopGamepadProbe();
        if (window.CharaBreakGamepadDebug) {
            delete window.CharaBreakGamepadDebug;
        }
    ...
    updateMissionHud(force = false) {
        ...
        if (this.missionStats) {
            ...
            const actionCount = this.usesBloomAction({ charged: true }) ? this.stageStats.playerBloomed : this.stageStats.playerBroken;
            this.missionStats.innerHTML = `
                ...
                <div><dt>${actionLabel}</dt><dd>${actionCount}</dd></div>
    ...
    startRunner(targets, onHit, onTextBreak, onImageHit, onGateHit, onEnemyHit) {
        ...
        this.root.addEventListener('pointerdown', this.handleStagePointerDown, { passive: false });
        this.root.addEventListener('pointermove', this.handleStagePointerMove, { passive: false });
        this.root.addEventListener('pointerup', this.handleStagePointerUp, { passive: false });
        this.root.addEventListener('pointercancel', this.handleStagePointerCancel, { passive: false });
        this.root.addEventListener('touchstart', this.handleStageTouchStart, { passive: false });
        this.root.addEventListener('touchmove', this.handleStageTouchMove, { passive: false });
        this.root.addEventListener('touchend', this.handleStageTouchEnd, { passive: false });
        this.root.addEventListener('touchcancel', this.handleStageTouchCancel, { passive: false });
        this.renderRunner();

        const showFullIntro = this.shouldShowIntroModal();
        this.runner.classList.add('gw-pixel-runner--intro-wait');
        this.showIntroModal((wantsBgm) => {
            if (showFullIntro) {
                this.markIntroSeen();
            }
            this.beginRunnerPlay({ showGuide: !showFullIntro, fromIntro: true });
        }, { fullIntro: showFullIntro });
    }
    ...
    showIntroModal(onStart, options = {}) {
        ...
        const showFullIntro = options.fullIntro !== false;
        const introLead = this.objectiveConfig.type === 'products'
            ...
            : UI_TEXT.introLead;
        const title = showFullIntro ? UI_TEXT.introTitle : UI_TEXT.bgmChoiceTitle;
        const lead = showFullIntro ? introLead : UI_TEXT.bgmChoiceLead;
        const keys = showFullIntro ? `<div class="gw-intro-modal__keys">${UI_TEXT.introKeys}</div>` : '';
        const pad = showFullIntro ? UI_TEXT.introPad : UI_TEXT.bgmChoiceNote;
        this.introModal.innerHTML = `
            <div class="gw-intro-modal__panel">
                <span class="gw-intro-modal__badge">${UI_TEXT.introBadge}</span>
                <h2 id="gw-intro-title">${title}</h2>
                <p>${escapeHtml(lead)}</p>
                ${keys}
                <small>${pad}</small>
                <div class="gw-intro-modal__audio-choice" role="group" aria-label="BGM">
                    <button type="button" class="gw-intro-modal__start gw-intro-modal__start--quiet" data-gw-intro-start data-gw-bgm-choice="0">${UI_TEXT.bgmChoiceOff}</button>
                    <button type="button" class="gw-intro-modal__start gw-intro-modal__start--music" data-gw-intro-start data-gw-bgm-choice="1">${UI_TEXT.bgmChoiceOn}</button>
                </div>
            </div>
        `;
        this.root?.appendChild(this.introModal);

        const startButtons = Array.from(this.introModal.querySelectorAll('[data-gw-intro-start]'));
        const startButton = startButtons[0] || null;
        const start = (event) => {
            event?.preventDefault?.();
            if (!this.introModal) {
                return;
            ...
            const wantsBgm = event?.currentTarget?.dataset?.gwBgmChoice === '1';
            this.onBgmChoice(Boolean(wantsBgm));
    ...
    startIntroGamepadPolling(onStart) {
        ...
        const tick = () => {
            ...
            const pressed = gamepadAnyButtonPressed(pad, [0, 1, 2, 3, 8, 9], 0.16);
    ...
    beginRunnerPlay(options = {}) {
        ...
        const startHint = isCoarsePointer() ? UI_TEXT.touchGuide : UI_TEXT.playerHint;
        this.showMessage(options.fromIntro ? UI_TEXT.introDrop : startHint, options.fromIntro ? 1500 : 1900);
        this.runnerLastTime = 0;
    ...
    stopRunner() {
        ...
        this.root?.removeEventListener('pointerdown', this.handleStagePointerDown);
        this.root?.removeEventListener('pointermove', this.handleStagePointerMove);
        this.root?.removeEventListener('pointerup', this.handleStagePointerUp);
        this.root?.removeEventListener('pointercancel', this.handleStagePointerCancel);
        this.root?.removeEventListener('touchstart', this.handleStageTouchStart);
        this.root?.removeEventListener('touchmove', this.handleStageTouchMove);
        this.root?.removeEventListener('touchend', this.handleStageTouchEnd);
        this.root?.removeEventListener('touchcancel', this.handleStageTouchCancel);
    ...
    handleStagePointerDown(event) {
        if (!this.isGameplayPointer(event)) {
            return;
        }

        if (event.pointerType !== 'mouse' && !event.isTouchFallback) {
            this.lastPointerTouchAt = currentTime();
        }
        event.preventDefault();
        event.stopPropagation();
        this.primeGamepadControls();
        ...
        this.setRunnerDirectionFromClientX(event.clientX);

        try {
            this.root?.setPointerCapture?.(event.pointerId);
        } catch (error) {
            // Pointer capture is best-effort; older mobile browsers can ignore it.
        }

        this.clearTouchLongPressTimer();
        this.touchLongPressTimer = window.setTimeout(() => {
            const gesture = this.touchGesture;
            if (!gesture || gesture.swiped || gesture.moved || gesture.longTriggered) {
                return;
            }

            gesture.longTriggered = true;
            this.clearPendingStageTap();
            this.runnerTouchKeys.delete('left');
            this.runnerTouchKeys.delete('right');
            this.releaseScrollPin();
            if (this.collectTreasureLetter({ silentNoLetter: true })) {
                gesture.longAction = 'letter';
                this.setThrowControlCharging(true, true);
            } else if (this.startAttackCharge('touch-stage')) {
                gesture.longAction = 'attack';
                this.setThrowControlCharging(false, false);
            }
            this.timers.delete(this.touchLongPressTimer);
            this.touchLongPressTimer = 0;
        }, TOUCH_LONG_PRESS_MS);
        this.timers.add(this.touchLongPressTimer);
    }

    handleStagePointerMove(event) {
        if (this.touchPointerId !== event.pointerId || !this.touchGesture || !this.isGameplayPointer(event)) {
            return;
        }
        ...
        const elapsed = currentTime() - gesture.startAt;
        const horizontalSpeed = absX / Math.max(elapsed, 1);
        const upwardIntent = dy <= -TOUCH_SWIPE_MIN_PX && absY > absX * 1.05;
        const downwardChargeIntent = dy >= TOUCH_CHARGE_PULL_PX && absY > absX * 1.05;
        const horizontalIntent = absX >= TOUCH_MOVE_HOLD_PX && absX > absY * 0.75;
        gesture.lastX = event.clientX;
        gesture.lastY = event.clientY;

        if (distance > TOUCH_TAP_MAX_MOVE) {
            gesture.moved = true;
            this.clearTouchLongPressTimer();
        }
        ...
        if (!gesture.longTriggered
            && !gesture.swiped
            && !gesture.jumped
            && downwardChargeIntent) {
            gesture.longTriggered = true;
            gesture.longAction = 'attack';
            gesture.chargePulled = true;
            gesture.swiped = true;
            this.clearPendingStageTap();
            this.runnerTouchKeys.delete('left');
            this.runnerTouchKeys.delete('right');
            ...
            this.releaseScrollPin();
            this.setThrowControlCharging(false, false);
            this.startAttackCharge('touch-pull');
            return;
        }
        ...
        if (!gesture.longTriggered
            ...
            this.runnerTouchKeys.delete('right');
            this.clearTouchLongPressTimer();
            this.pulseTouchControl('jump', 220);
            return;
        }
        ...
        if (!gesture.longTriggered
            && !gesture.swiped
            && horizontalIntent
            && distance >= TOUCH_PARRY_MIN_PX
            && elapsed <= TOUCH_PARRY_MAX_MS
            && horizontalSpeed >= TOUCH_PARRY_MIN_SPEED) {
            gesture.swiped = true;
            this.runnerTouchKeys.delete('left');
            this.runnerTouchKeys.delete('right');
            this.clearTouchLongPressTimer();
            this.triggerTouchParry(dx, dy);
            return;
        }

        if (gesture.longTriggered || gesture.swiped) {
            return;
        }

        if (horizontalIntent) {
            this.runnerTouchKeys.delete(dx < 0 ? 'right' : 'left');
            this.runnerTouchKeys.add(dx < 0 ? 'left' : 'right');
            this.setRunnerDirectionFromClientX(event.clientX);
            this.releaseScrollPin();
            return;
        }
    ...
    handleStagePointerUp(event) {
        if (this.touchPointerId !== event.pointerId || !this.touchGesture) {
            return;
        }
        ...
        const dy = event.clientY - gesture.startY;
        const distance = Math.hypot(dx, dy);
        const longTriggered = gesture.longTriggered;
        const longAction = gesture.longAction;
        const chargePulled = Boolean(gesture.chargePulled);
        const swiped = gesture.swiped;
        ...
        if (longTriggered) {
            if (longAction === 'attack') {
                const releaseDownward = chargePulled
                    || (dy >= TOUCH_CHARGE_PULL_PX * 0.65 && Math.abs(dy) > Math.abs(dx) * 0.85);
                this.finishAttackCharge({ forceCharged: chargePulled || releaseDownward, downward: releaseDownward });
            } else {
                this.finishTouchLetterHold();
            }
            return;
        }

        if (swiped || distance > TOUCH_TAP_MAX_MOVE) {
            return;
        }

        this.handleStageTap(event.clientX, event.clientY);
    }
    ...
    handleStagePointerCancel(event) {
        ...
        const longAction = this.touchGesture?.longAction;
        this.releaseStagePointer(event);
        this.clearTouchLongPressTimer();
        this.runnerTouchKeys.delete('left');
        this.runnerTouchKeys.delete('right');
    ...
    handleStageTouchStart(event) {
        ...
        if (!touch || this.shouldIgnoreTouchFallback()) {
            return;
        }

        this.handleStagePointerDown(this.touchAsPointerEvent(event, touch));
    }

    handleStageTouchMove(event) {
        const touch = this.changedTouchForActiveGesture(event);
        if (!touch || this.shouldIgnoreTouchFallback()) {
            return;
        }

        this.handleStagePointerMove(this.touchAsPointerEvent(event, touch));
    }
    ...
    handleStageTouchEnd(event) {
        ...
        this.handleStagePointerUp(this.touchAsPointerEvent(event, touch));
    }

    handleStageTouchCancel(event) {
        const touch = this.changedTouchForActiveGesture(event);
        if (!touch || this.shouldIgnoreTouchFallback()) {
            return;
        }

        this.handleStagePointerCancel(this.touchAsPointerEvent(event, touch));
    }

    isGameplayPointer(event) {
        if (!this.runnerState || this.stageCleared || this.stageFailed || this.introModal) {
            return false;
        }
        ...
        if (target?.closest?.('.gw-controls, .gw-player-controls, .gw-intro-modal, .gw-world-map, .gw-end-roll, .gw-game-over')) {
            return false;
        }
    ...
    shouldIgnoreTouchFallback() {
        return typeof window.PointerEvent === 'function'
            && currentTime() - this.lastPointerTouchAt < 520;
    }

    changedTouchForActiveGesture(event) {
        if (this.touchPointerId === null) {
            return event.changedTouches?.[0] || null;
        }

        const activeIdentifier = String(this.touchPointerId).replace(/^touch-/, '');
        return Array.from(event.changedTouches || [])
            .find((touch) => String(touch.identifier) === activeIdentifier) || null;
    }

    touchAsPointerEvent(event, touch) {
        return {
    ...
    clearTouchGestureState() {
        this.clearTouchLongPressTimer();
        this.clearPendingStageTap();
        if (this.touchParryTimer) {
            window.clearTimeout(this.touchParryTimer);
            this.timers.delete(this.touchParryTimer);
            this.touchParryTimer = 0;
        }
        this.touchParryUntil = 0;
        this.touchGesture = null;
        this.touchPointerId = null;
        this.runnerTouchKeys.clear();
        this.root?.classList.remove('gw-stage--touch-parry');
    }
    ...
    clearPendingStageTap() {
        ...
        window.clearTimeout(this.touchTapTimer);
        this.timers.delete(this.touchTapTimer);
        this.touchTapTimer = 0;
    }

    handleStageTap(clientX, clientY) {
        if (!this.runnerState) {
            return;
        }

        const now = currentTime();
        const doubleTap = this.lastTouchTapAt
            && now - this.lastTouchTapAt <= TOUCH_DOUBLE_TAP_MS
            && Math.hypot(clientX - this.lastTouchTapX, clientY - this.lastTouchTapY) <= 34;

        this.setRunnerDirectionFromClientX(clientX);
        ...
        this.clearPendingStageTap();
        this.touchTapTimer = window.setTimeout(() => {
            this.touchTapTimer = 0;
            this.lastTouchTapAt = 0;
            this.releaseScrollPin();
            this.runnerAttack();
        }, TOUCH_SINGLE_TAP_DELAY_MS);
        this.timers.add(this.touchTapTimer);
    }

    setRunnerDirectionFromClientX(clientX) {
        if (!this.runnerState) {
            return;
        }

        const runnerCenter = this.runnerState.x + RUNNER_WIDTH / 2;
        if (clientX < runnerCenter - 10) {
            this.runnerState.direction = -1;
        } else if (clientX > runnerCenter + 10) {
            this.runnerState.direction = 1;
        }
    }

    triggerTouchParry(dx = 0, dy = 0) {
        if (!this.runnerState) {
            return;
        }

        this.touchParryUntil = currentTime() + TOUCH_PARRY_MS;
        this.runnerTouchKeys.add('shield');
        this.root?.classList.add('gw-stage--touch-parry');
        this.releaseScrollPin();
        this.setRunnerDirectionFromClientX(this.runnerState.x + RUNNER_WIDTH / 2 + dx);
        this.impactBurstAt(this.runnerShieldRect(), 'soft');
        this.shakeScreen('soft');
        this.onRunnerSound('uiMove', { volume: 0.08, rate: dy < 0 ? 1.18 : 1.05 });
        this.showMessage(UI_TEXT.touchParry, 700);
        ...
        this.touchParryTimer = window.setTimeout(() => {
            this.runnerTouchKeys.delete('shield');
            this.root?.classList.remove('gw-stage--touch-parry');
            this.timers.delete(this.touchParryTimer);
            this.touchParryTimer = 0;
        }, TOUCH_PARRY_MS);
        this.timers.add(this.touchParryTimer);
    }

    pulseTouchControl(control, duration = 160) {
        this.runnerTouchKeys.add(control);
        this.releaseScrollPin();
        const timer = window.setTimeout(() => {
            this.runnerTouchKeys.delete(control);
            this.timers.delete(timer);
        }, duration);
        this.timers.add(timer);
    }

    finishTouchLetterHold() {
        this.setThrowControlCharging(false, false);
    ...
    mobileOpenPlayBounds() {
        const edgeAllowance = RUNNER_WIDTH * 0.45;
        return {
            left: -edgeAllowance,
            right: Math.max(window.innerWidth + edgeAllowance, RUNNER_WIDTH + edgeAllowance),
        };
    }

    calculatePlayBounds() {
        if (this.usesMobileOpenSides()) {
            return this.mobileOpenPlayBounds();
        }
    ...
    pickAndThrowAtLock(options = {}) {
        ...
        if (this.heldLetter) {
            return this.launchHeldProjectile();
        }
    ...
    bloomPlacedLettersAtRect(rect, options = {}) {
        this.syncPlacedLettersToScroll();
        ...
        for (const placed of this.placedLetters) {
            if (!placed.element?.isConnected || placed.bloomed) {
                continue;
            }
        ...
        candidates.sort((a, b) => a.distance - b.distance || b.overlap - a.overlap);
        const bloomed = candidates.slice(0, limit);

        if (bloomed.length === 0) {
            return null;
        }

        const bloomedSet = new Set(bloomed.map((item) => item.placed));
        this.placedLetters = this.placedLetters.filter((placed) => !bloomedSet.has(placed));

        for (const item of bloomed) {
            item.placed.bloomed = true;
            item.placed.element.classList.add('gw-held-letter--bloomed');
            item.placed.element.style.setProperty('--gw-bloom-color', bloomColorForChar(item.placed.char));
            item.placed.element.textContent = bloomGlyphForChar(item.placed.char);
            this.removeLater(item.placed.element, this.reducedMotion ? 180 : 620);
        }

        this.rebuildRoomCollisionCache();

        return {
            count: bloomed.length,
            chars: bloomed.map((item) => item.placed.char),
            items: bloomed.map((item) => ({
                char: item.placed.char,
                rect: item.rect,
                target: null,
                critical: false,
            ...
            criticalWord: '',
            target: null,
            rect: unionRects(bloomed.map((item) => item.rect)) || rect,
        };
    }

    bloomPlacedLettersAtRects(rects = [], options = {}) {
        const results = [];
        let remaining = options.limit || rects.length || 1;
        const rectLimits = Array.isArray(options.rectLimits) ? options.rectLimits : [];
        ...
        for (let index = 0; index < rects.length && remaining > 0; index += 1) {
            const rectLimit = Math.max(1, rectLimits[index] || options.perRectLimit || 1);
            const result = this.bloomPlacedLettersAtRect(rects[index], {
                ...options,
                limit: Math.min(rectLimit, remaining),
            });
            if (result?.count > 0) {
    ...
    bloomTextAtRects(rects = [], options = {}) {
        if (!this.textBreaker?.bloomAtRect) {
            return null;
        }
        ...
        for (let index = 0; index < rects.length && remaining > 0; index += 1) {
            const rectLimit = Math.max(1, rectLimits[index] || options.perRectLimit || 1);
            const result = this.textBreaker.bloomAtRect(rects[index], {
                ...options,
                limit: Math.min(rectLimit, remaining),
            });
            if (result?.count > 0) {
    ...
    usesBloomAction(options = {}) {
        if (this.playStyle === 'bloom') {
            return true;
        }

        if (this.playStyle === 'hybrid') {
            return Boolean(options.charged);
        }

        return false;
    }

    usesFlowerEnergyProjectile() {
        return this.playStyle === 'bloom' || this.playStyle === 'hybrid';
    }

    handleBloomResult(result, options = {}) {
        if (!result?.count) {
            return false;
        }

        this.bumpStageStat('playerBloomed', result.count);
        this.bloomBurstAt(result.rect, options.intensity || 'medium', result.chars?.[0] || '');
        this.plantWorldFlowers(result.rect, result.count, { chars: result.chars || [] });
        this.rebuildRoomCollisionCache();
        this.shakeScreen(options.full ? 'soft' : 'soft');
        this.onRunnerSound('magicWand', {
            volume: options.full ? 0.09 : 0.065,
            rate: options.full ? 0.95 : 1.08,
        });
        this.onRunnerSound(options.full ? 'bloomFull' : 'bloom', {
            volume: options.full ? 0.11 : 0.082,
            rate: options.full ? 0.94 : 1.04,
        });

        if (this.stageStats.playerBloomed >= 8) {
            this.root?.classList.add('gw-stage--bloom-field');
        }

        this.showMessage(options.message || UI_TEXT.runnerBloom, options.full ? 1400 : 1200);
        return true;
    }
    ...
    launchHeldProjectile(options = {}) {
        return this.usesFlowerEnergyProjectile()
            ? this.launchFlowerEnergyProjectile(options)
            : this.launchWordProjectile(options);
    }

    launchFlowerEnergyProjectile(options = {}) {
        ...
        const color = bloomColorForChar(held.char);
        this.clearTreasureStoreTimer();
        const handRect = this.runnerHandRect();
        ...
        const targetX = targetRect
            ? targetRect.left + targetRect.width / 2
            : startX + this.runnerState.direction * 620;
        ...
        const distance = Math.max(1, Math.hypot(dx, dy));
        const speed = this.reducedMotion ? FLOWER_ARROW_SPEED * 1.16 : FLOWER_ARROW_SPEED;
        const element = held.element;
        element.className = `gw-flower-arrow gw-flower-arrow--power-${power.name}`;
        element.textContent = '';
        element.removeAttribute('aria-label');
        for (const property of ['color', 'fontFamily', 'fontSize', 'fontStyle', 'fontWeight', 'letterSpacing', 'lineHeight', 'textShadow']) {
            element.style.removeProperty(property.replace(/[A-Z]/g, (match) => `-${match.toLowerCase()}`));
        }
        element.style.setProperty('--gw-flower-color', color);
        element.style.setProperty('--gw-flower-size', `${clamp(Math.max(held.width, held.height) * 1.22, 26, 66)}px`);
        element.style.width = 'var(--gw-flower-size)';
        element.style.height = 'calc(var(--gw-flower-size) * 0.32)';
        ...
        this.projectiles.push({
            ...
            speed,
            age: 0,
            lifetime: FLOWER_ARROW_LIFETIME,
            ...
            power: power.name,
            hitbox: clamp(Math.max(held.width, held.height) + power.hitboxBonus + 10, power.hitboxMin + 8, 128),
        });

        this.bloomBurstAt(handRect, 'soft', held.char);
        this.onRunnerSound('flowerArrow', { volume: power.throwVolume * 0.72, rate: power.rate });
        this.showMessage(UI_TEXT.flowerArrow, 900);
        return true;
    }
    ...
    updateProjectiles(delta) {
        ...
        for (const projectile of this.projectiles) {
            ...
            if (targetRect) {
                ...
                const distance = Math.max(1, Math.hypot(dx, dy));
                const projectileSpeed = projectile.speed || WORD_THROW_SPEED;
                projectile.vx = lerp(projectile.vx, (dx / distance) * projectileSpeed, 0.26);
                projectile.vy = lerp(projectile.vy, (dy / distance) * projectileSpeed, 0.26);
            }
            ...
            if (chestHit) {
                projectile.element.style.setProperty('--gw-hit-rotate', `${angle}deg`);
                projectile.element.classList.add(projectile.kind === 'flower' ? 'gw-flower-arrow--hit' : 'gw-word-shot--hit');
                if (projectile.kind === 'flower') {
                    this.bloomBurstAt(shotRect, projectile.intensity || 'medium', projectile.char);
                }
            ...
            if (projectile.age > (projectile.lifetime || WORD_THROW_LIFETIME) || outOfView) {
                projectile.element.remove();
                continue;
            }
    ...
    maxActiveEnemyMissiles() {
        if (window.innerWidth < 720 || this.reducedMotion) {
            return BOSS_MISSILE_MAX_ACTIVE_MOBILE;
        }

        return BOSS_MISSILE_MAX_ACTIVE;
    }

    warnAndFireBossMissile(enemy) {
        if (!enemy || enemy.type !== 'boss' || enemy.defeated || this.enemyMissiles.length >= this.maxActiveEnemyMissiles()) {
            return;
        }
        ...
        const timer = window.setTimeout(() => {
            ...
            if (!this.stageCleared && !this.stageFailed && !enemy.defeated && this.enemyMissiles.length < this.maxActiveEnemyMissiles()) {
    ...
    fireEnemyMissile(enemy) {
        if (!this.projectileLayer || !enemy?.element?.isConnected || enemy.type !== 'boss' || this.enemyMissiles.length >= this.maxActiveEnemyMissiles()) {
            return;
        }
    ...
    parryMissile(missile) {
        ...
        const rect = this.missileRect(missile);
        this.impactBurstAt(rect, this.parryCombo > 1 ? 'hard' : 'medium');
        this.spawnParryFlash(rect, this.parryCombo);
        this.shakeScreen('soft');
        this.onRunnerSound('parry', {
            volume: Math.min(0.23, 0.14 + this.parryCombo * 0.018),
            rate: Math.min(1.28, 1.06 + this.parryCombo * 0.035),
        });
        this.showMessage(this.parryCombo > 1 ? `${UI_TEXT.missileParry} x${this.parryCombo}` : UI_TEXT.missileParry, 1100);
    }

    spawnParryFlash(rect, combo = 1) {
        const centerX = rect.left + rect.width / 2;
        const centerY = rect.top + rect.height / 2;
        const flash = document.createElement('span');
        flash.className = 'gw-parry-flash';
        ...
        label.className = 'gw-parry-label';
    ...
    stageResult(reason = '') {
        return {
            ...
            player_bloomed_count: this.stageStats.playerBloomed,
    ...
    controlHeld(control) {
        if (control === 'shield' && currentTime() < this.touchParryUntil) {
            return true;
        }

        return this.runnerKeys.has(control)
            || this.runnerTouchKeys.has(control)
            || this.runnerGamepadKeys.has(control);
    }
    ...
    updateGamepadControls() {
        ...
        this.setGamepadHeld('jump', direction.up || gamepadAnyButtonPressed(pad, [0, 12], 0.18));
        this.setGamepadHeld('shield', gamepadAnyButtonPressed(pad, [4, 6, 7, 10, 11], 0.18));
        this.handleGamepadChargeButton('attack', gamepadAnyButtonPressed(pad, [1, 2], 0.18));
        this.handleGamepadThrowButton(gamepadAnyButtonPressed(pad, [3, 5], 0.18));
        this.handleGamepadButton('lock', gamepadAnyButtonPressed(pad, [9], 0.18), () => this.cycleLockOn());
        this.handleGamepadButton('map', gamepadAnyButtonPressed(pad, [8], 0.18), () => {
    ...
    startGamepadProbe() {
        ...
        const tick = (time) => {
            ...
            if (!this.gamepadProbeLastAt || time - this.gamepadProbeLastAt > 420) {
                this.gamepadProbeLastAt = time;
                const wasAvailable = this.gamepadWasAvailable;
                const pad = this.readActiveGamepad({ allowIdle: true });
                if (pad) {
                    this.activeGamepadIndex = pad.index;
                    this.gamepadWasAvailable = true;
                    if (!wasAvailable) {
                        this.showMessage(UI_TEXT.gamepadConnected, 1000);
                    }
                }
            }

            this.gamepadProbeFrame = requestFrame(tick);
        };
    ...
    stopGamepadProbe() {
        if (!this.gamepadProbeFrame) {
            return;
        }

        cancelFrame(this.gamepadProbeFrame);
        this.gamepadProbeFrame = 0;
        this.gamepadProbeLastAt = 0;
    }
    ...
    describeGamepadState() {
        const navigatorRef = window.navigator || (typeof navigator !== 'undefined' ? navigator : null);
        const hasApi = Boolean(navigatorRef && typeof navigatorRef.getGamepads === 'function');
        let pads = [];

        if (hasApi) {
            try {
                pads = Array.from(navigatorRef.getGamepads()).filter(Boolean).map((pad) => ({
                    index: pad.index,
                    id: pad.id,
                    ...
                    axes: Array.from(pad.axes || []).map((axis) => Number(axis || 0)),
                }));
            } catch (error) {
                pads = [{ error: error?.message || String(error) }];
            }
        }
        ...
        return {
            ...
            hasGamepadApi: hasApi,
            activeGamepadIndex: this.activeGamepadIndex,
            gamepadWasAvailable: this.gamepadWasAvailable,
            pads,
        };
    }

    isGamepadActive(gamepad) {
        const hasPressedButton = Array.from(gamepad.buttons || []).some((button) => gamepadButtonActive(button, 0.16));
        if (hasPressedButton) {
            return true;
        }
        ...
        return Array.from(gamepad.axes || []).some((axis, index) => {
            ...
            return Number.isFinite(value) && Math.abs(value) > 0.18 && Math.abs(value) <= 1.05;
        });
    }
    ...
    finishAttackCharge(options = {}) {
        ...
        const rawDuration = currentTime() - charge.startedAt;
        const duration = Math.min(
            options.forceCharged ? Math.max(rawDuration, CHARGE_READY_MS) : rawDuration,
            CHARGE_MAX_MS,
        );
        this.clearAttackCharge();
        ...
        if (duration >= CHARGE_READY_MS) {
            this.runnerChargedAttack(duration, { downward: Boolean(options.downward) });
        } else {
            this.runnerAttack();
        }
    ...
    runnerAttack() {
        ...
        if (this.usesBloomAction({ charged: false })) {
            const placedBloom = this.bloomPlacedLettersAtRect(attackRect, { limit: 1 });
            if (placedBloom?.count > 0) {
                this.handleBloomResult(placedBloom, { intensity: 'medium' });
                return;
            }
        } else {
        ...
        if (this.usesBloomAction({ charged: false })) {
            const charBloom = this.textBreaker?.bloomAtRect(attackRect, { limit: 1, source: 'player' });
            if (charBloom?.count > 0) {
                this.handleBloomResult(charBloom, { intensity: 'medium' });
                return;
            }
        } else {
    ...
    runnerChargedAttack(duration, options = {}) {
        if (!this.runnerState) {
            return;
        }
        ...
        if (this.usesBloomAction({ charged: true })) {
            const placedBloom = this.bloomPlacedLettersAtRects(chargedRects, {
                limit: breakLimit,
                rectLimits: breakRectLimits,
            });
            if (placedBloom?.count > 0) {
                this.handleBloomResult(placedBloom, {
                    intensity,
                    full,
                    message: full ? UI_TEXT.bloomField : UI_TEXT.chargeBloom,
                });
                return;
            }
        } else {
        ...
        if (this.usesBloomAction({ charged: true })) {
            const charBloom = this.bloomTextAtRects(chargedRects, {
                limit: breakLimit,
                rectLimits: breakRectLimits,
                source: 'player',
            });
            if (charBloom?.count > 0) {
                this.handleBloomResult(charBloom, {
                    intensity,
                    full,
                    message: full ? UI_TEXT.bloomField : UI_TEXT.chargeBloom,
                });
    ...
    runnerChargedAttackRects(full = false, options = {}) {
        ...
        if (options.downward) {
            ...
            rects.unshift({
                ...
                width: downWidth,
                height: downHeight,
                right: downLeft + downWidth,
                bottom: downTop + downHeight,
            });
        }
    ...
    bloomBurstAt(rect, intensity = 'medium', char = '') {
        if (!rect) {
            return;
        }
        ...
        for (let index = 0; index < petalCount; index += 1) {
            ...
            petal.className = `gw-bloom-petal gw-bloom-petal--${index % 4}`;
            petal.textContent = index % 3 === 0 ? bloomGlyphForChar(`${char}${index}`) : '\u2726';
            petal.style.left = `${centerX + randomBetween(-14, 14)}px`;
            petal.style.top = `${centerY + randomBetween(-12, 12)}px`;
            petal.style.setProperty('--gw-bloom-color', bloomColorForChar(`${char}${index}`));
            petal.style.setProperty('--gw-x', `${randomBetween(-110, 110)}px`);
    ...
    plantWorldFlowers(rect, count = 1, options = {}) {
        if (!this.effectLayer || !rect || this.reducedMotion) {
            return;
        }
        ...
        for (let index = 0; index < flowerCount; index += 1) {
            const char = chars[index % Math.max(chars.length, 1)] || `${centerX}${index}`;
            const flower = document.createElement('span');
            flower.className = `gw-world-flower gw-world-flower--${index % 5}`;
            flower.textContent = bloomGlyphForChar(char);
            flower.style.setProperty('--gw-bloom-color', bloomColorForChar(`${char}${index}`));
            flower.style.setProperty('--gw-scale', String(randomBetween(0.78, 1.18)));
            flower.style.setProperty('--gw-d', `${randomBetween(0, 160)}ms`);

            const sideX = preferRight
                ? randomBetween(window.innerWidth - laneMax, window.innerWidth - laneMin)
                : randomBetween(laneMin, laneMax);
            const nearImpactX = clamp(centerX + randomBetween(-52, 52), laneMin, window.innerWidth - laneMin);
            const useLane = index % 3 !== 0 || rect.width > window.innerWidth * 0.5;
            flower.style.left = `${useLane ? sideX : nearImpactX}px`;
            flower.style.top = `${clamp(centerY + randomBetween(34, 156), 84, window.innerHeight - 32)}px`;

            this.bloomFlowers.push(flower);
            this.effectLayer.appendChild(flower);
        }

        while (this.bloomFlowers.length > maxFlowers) {
            this.bloomFlowers.shift()?.remove();
        }
    }
    ...
    removeLater(element, lifetime) {
        const timer = window.setTimeout(() => {
            element.remove();
            this.temporaryEffects = this.temporaryEffects.filter((item) => item !== element);
            this.timers.delete(timer);
        }, lifetime);
...
function normalizePlayStyle(value) {
    const style = String(value || 'break').toLowerCase();
    return ['break', 'bloom', 'hybrid'].includes(style) ? style : 'break';
}

function bloomGlyphForChar(char = '') {
    const glyphs = ['\u273f', '\u2740', '\u2735', '\u2726'];
    const code = Array.from(String(char || ' ')).reduce((total, part) => total + part.codePointAt(0), 0);
    return glyphs[Math.abs(code) % glyphs.length];
}

function bloomColorForChar(char = '') {
    const code = Array.from(String(char || ' ')).reduce((total, part) => total + part.codePointAt(0), 0);
    return BLOOM_COLORS[Math.abs(code) % BLOOM_COLORS.length];
}
...
function gamepadButtonActive(button, threshold = 0.16) {
    return Boolean(button?.pressed) || Number(button?.value || 0) > threshold;
}

function gamepadButtonPressed(gamepad, index, threshold = 0.16) {
    return gamepadButtonActive(gamepad?.buttons?.[index], threshold);
}

function gamepadAnyButtonPressed(gamepad, indexes = [], threshold = 0.16) {
    return indexes.some((index) => gamepadButtonPressed(gamepad, index, threshold));
}
...
function gamepadDirection(gamepad) {
    ...
    for (const [xIndex, yIndex] of axisPairs) {
        const x = gamepadAxis(gamepad, xIndex);
        const y = gamepadAxis(gamepad, yIndex);
        axisLeft = axisLeft || x < -0.22;
        axisRight = axisRight || x > 0.22;
        axisUp = axisUp || y < -0.42;
        axisDown = axisDown || y > 0.42;
    }

    const hat = gamepadHatDirection(gamepadAxis(gamepad, 9));

    return {
        left: axisLeft || gamepadButtonPressed(gamepad, 14, 0.16) || hat.left,
        right: axisRight || gamepadButtonPressed(gamepad, 15, 0.16) || hat.right,
        up: axisUp || gamepadButtonPressed(gamepad, 12, 0.16) || hat.up,
        down: axisDown || gamepadButtonPressed(gamepad, 13, 0.16) || hat.down,
    };
}
