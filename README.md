...
This is the current public CharaBreak site running in game mode:

![CharaBreak live gameplay on charabreak.com](screenshots/charabreak-live-gameplay.png)
...
The current public package enforces this boundary in the plugin itself. On the Free plan, CharaBreak only loads the game on the first enabled stage, hides the multi-page world map, and locks custom enemy assignment, stage-specific audio, and advanced product/item objectives.
...
When a valid Freemius Pro license or trial is active, the same plugin unlocks the Pro feature set automatically. No separate plugin folder should be installed; keep the plugin directory and ZIP name as `charabreak`.
...
On phones, CharaBreak also switches to an ultra-light performance profile. It limits wrapped text, shortens or removes nonessential particles, disables page-wide shake, caches collision rectangles, checks only the active room for text/image hits, and throttles background systems such as enemy/chest/gate checks so the runner input stays smooth on long pages.
...
現在の公開パッケージでは、無料版は最初に有効化した1ステージだけがゲームとして読み込まれます。ワールドマップ、敵キャラ台帳、ステージごとのBGM、商品・アイテム収集条件などはPro機能としてロックされます。

Pro版では、複数ページのワールドマップ化、ステージごとのBGM、敵キャラ台帳、ボス、報酬、クーポン、限定URL、Break / Bloom / Hybridのステージ別設定などを開放します。Freemiusで有効なProライセンスまたはトライアルが入ると、同じ `charabreak` プラグイン内で自動的に解放されます。
